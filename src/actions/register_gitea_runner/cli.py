...
from utilities.click import CONTEXT_SETTINGS, ListStrs, Str
...
def make_register_gitea_runner_cmd(
    *, cli: Callable[..., Command] = command, name: str | None = None
) -> Command:
    @option("--runner-token", type=Str(), default=None, help="Registration token")
    ...
    @option(
        "--runner-capacity", type=int, default=RUNNER_CAPACITY, help="Runner capacity"
    )
    @option(
        "--runner-labels", type=ListStrs(), default=RUNNER_LABELS, help="Runner labels"
    )
    @option(
